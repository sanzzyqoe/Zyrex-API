...
app.use((req, res, next) => {
  ...
  const validApiKeys = settings.apiSettings.apiKeys || [];
  
  // Skip API key check for certain endpoints
  const skipPaths = ["/api/settings", "/assets/", "/src/", "/api/preview-image", "/api/notifications"];
  const shouldSkip = skipPaths.some((path) => req.path.startsWith(path));

  if (req.path.startsWith('/api/') && !shouldSkip && !validApiKeys.includes(apiKey)) {
    return res.status(401).sendFile(path.join(__dirname, 'api-page', '401.html'));
  }
  next();
});
...
app.get("/", (req, res) => {
  res.sendFile(path.join(__dirname, "api-page", "index-new.html"))
})

app.get("/assets/styles-new.css", (req, res) => {
  res.setHeader("Content-Type", "text/css")
  res.sendFile(path.join(__dirname, "api-page", "styles-new.css"))
})

app.get("/assets/script-new.js", (req, res) => {
  res.setHeader("Content-Type", "application/javascript")
  res.sendFile(path.join(__dirname, "api-page", "script-new.js"))
})
